...
/**
    ...
    while (!$categories->EOF)  
    {
      ...
      $this->tree[$categories->fields['categories_id']] = 
      		array('name' => $categories->fields['categories_name'],
      			 ...
      			 'next_id' => false,
      			 'mainID' => $categories->fields['categories_id']);
  ...
  function zen_category_tree($product_type = "all") 
  {
    ...
    {
      $categories_query = "select 
      							ptc.category_id as categories_id, 
      							cd.categories_name, 
      							c.parent_id, 
      							c.categories_image
                           from 
                             	" . TABLE_CATEGORIES . " c, 
                             	" . TABLE_CATEGORIES_DESCRIPTION . " cd, 
                             	" . TABLE_PRODUCT_TYPES_TO_CATEGORY . " ptc
                           where 
                           		c.parent_id = 0
    ...
    //store all of the parent categories in the array
    //array[id] = array(name, parentid, level, path, image, next_id);
        ...
        if ($rows->RecordCount()>0) 
        {
          ...
          while (!$rows->EOF) 
          {
            ...
            $this->tree[$rows->fields['categories_id']] =
            	 array('name' => $rows->fields['categories_name'] . " : " . $key,
  ...
  function zen_show_all_categories($treeid, $boxArrayID)
  {
  	   $this->categories_string = "";
  	   
  
 
  	    	//parent id found so it is a child element
  	    	if ($this->tree[$treeid]['parent'] != 0) 
      		{
        		//$this->categories_string .= CATEGORIES_SUBCATEGORIES_INDENT;
      		}
      		
      		//no parent id found so it is a top element
      		if ($this->tree[$treeid]['parent'] == 0) 
    		{
    			$this->box_categories_array[$boxArrayID]['top'] = 'true';
    			$this->box_categories_array[$boxArrayID]['current'] = true;
    		}
    		else
    		{
    			$this->box_categories_array[$boxArrayID]['top'] = 'false';
    			//$this->categories_string .= CATEGORIES_SEPARATOR_SUBS;
    		}
    		
    		$this->box_categories_array[$boxArrayID]['id'] = $this->tree[$treeid]['mainID'];
    		$this->box_categories_array[$boxArrayID]['pid'] = $this->tree[$treeid]['parent'];
    		// display category name
    		$this->box_categories_array[$boxArrayID]['name'] = $this->categories_string . $this->tree[$treeid]['name'];
    		
    		// make category image available in case needed
    		$this->box_categories_array[$boxArrayID]['image'] = $this->tree[$treeid]['image'];
    		
    		if ($this->tree[$treeid]['has_sub_cat']) 
    		{
    		    	
      			$this->box_categories_array[$boxArrayID]['has_sub_cat'] = true;
      			
    		} 
    		else 
    		{
    			
      			$this->box_categories_array[$boxArrayID]['has_sub_cat'] = false;
      			
    		}
    
      	
  	    
  	        if ($this->tree[$treeid]['next_id'] != false) {
      			$boxArrayID++;
      			$this->zen_show_all_categories($this->tree[$treeid]['next_id'], $boxArrayID);
    		}		
